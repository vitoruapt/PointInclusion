function [in, on] = inpolygon2(x,y,xv,yv)
%INPOLYGON2 True for points inside or on a polygonal region.
%   IN = INPOLYGON2(X,Y,XV,YV) returns a matrix IN the size of X and Y.
...
%
%   Being adapted by vsantos for studies...
%
...
mask = (x >= min(xv)) & (x <= max(xv)) & (y>=min(yv)) & (y<=max(yv));  % __vs those on the bounding box of polygon
if ~any(mask)
    in = false(inputSize);
    on = in;
    return
...
% 10 Megabytes.
block_length = 1e5;  %%% THIS HAS INFLUENCE ON CODE SPEED __vs
